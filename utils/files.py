...
def load_tcp_targets(filepath: str = "tcp16.json") -> List[Any]:
    ...
    try:
        ...
        wait_and_exit()
def load_ai_domains(filepath: str = "domains_ai.txt") -> List[str]:
    """Загружает список AI/LLM сервисов для теста 8."""
    path = get_resource_path(filepath)

    if not path.exists():
        console.print(f"[yellow]Файл {filepath} не найден — тест 8 (AI) недоступен.[/yellow]")
        return []
